...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${t.dashboard.addBotPageTitle} - ${t.meta.appTitle}`,
    description: t.dashboard.addBotPageDescription,
  };
}

export default async function BotNewPage({ params }: PageProps) {
  const { locale } = await params;
  if (!isLocale(locale)) {
    notFound();
  }
  ...
  return (
    <RouteGuard locale={locale}>
      <BotUpsertFormPage locale={locale as Locale} mode="create" />
    </RouteGuard>
  );
}
